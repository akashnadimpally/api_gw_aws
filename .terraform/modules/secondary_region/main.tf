  ...
  api_methods = flatten([
    for resource in var.api_config.resources : [
      for method in resource.methods : {
        ...
        auth_type        = try(method.auth_type, "NONE")
        uri              = method.uri
      }
    ]
  ])
...
resource "aws_api_gateway_method" "method" {
  ...
  # Corrected reference using each.value
  authorization = upper(each.value.auth_type) == "COGNITO" ? "COGNITO_USER_POOLS" : upper(each.value.auth_type) == "CUSTOM" ? "CUSTOM" : "NONE"

  # Corrected authorizer reference
  authorizer_id = upper(each.value.auth_type) == "COGNITO" ? aws_api_gateway_authorizer.cognito.id :upper(each.value.auth_type) == "CUSTOM" ? aws_api_gateway_authorizer.custom.id :null
